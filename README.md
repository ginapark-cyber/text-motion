...
- 상단 바: **Undo / Redo**(⌘Z / ⇧⌘Z), **Reset**(현재 효과 설정만 기본값으로, 텍스트·서체는 유지), **프리셋 저장**, **Export**
- 왼쪽: 텍스트, 서체(패밀리·스타일·크기·색), 위치(정렬·상하·**Max width**), Exit. 각 섹션의 **More** 를 펴면 자간·행간·폭·이탤릭, 여백, 타이밍 같은 세부 조절이 나옴
- **Max width**: 텍스트가 이 폭(화면 폭의 %)을 넘으면 단어 단위로 자동 줄바꿈. 100% = 끔. 직접 넣은 줄바꿈은 항상 유지되고, 미리보기에 점선 가이드가 표시됨(내보내기엔 안 나감). 자동 줄바꿈으로 줄이 늘고 행간이 기본값(0.62)이면 0.95 로 렌더됨
- 가운데: 미리보기. 스페이스바 재생/정지, 슬라이더로 스크럽. 배경 토글은 확인용이고 내보내기엔 영향 없음
- 오른쪽: 출력 크기, 파일명(More: fps·형식), 효과 설정(More: 세부 파라미터), **Presets**
- **Presets**: 상단 바의 저장 아이콘 → 이름(기본 `효과 · 텍스트 앞 두 단어`, 바꿔도 됨) → 서버에 저장되어 팀 전체가 공유. 목록에서 클릭하면 적용, ✎ 이름 변경, × 삭제
- **Exit**: Reverse out 을 고르면 등장 효과가 끝나고 Hold 만큼 머문 뒤 **같은 효과를 거꾸로 재생**하며 사라짐 (모든 효과 공통). More 에서 사라지는 속도 조절
